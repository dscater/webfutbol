...
<body>
    ...
    @foreach ($equipos as $equipo)
        <div class="encabezado">
            ...
            <h4 class="texto">LISTA DE JUGADORES POR EQUIPO</h4>
            <h4 class="fecha">Expedido: {{ date('d-m-Y') }}</h4>
        </div>
        <h2 class="subtitulo centreado">{{ $equipo->nombre }}</h2>
        @if (count($equipo->fichajes) > 0)
            @php
                $fichajes = $equipo->fichajes;
            @endphp
            <table border="1">
                <thead>
                    <tr>
                ...
                <tbody>
                    @php
                        $cont = 1;
                    @endphp
                    @foreach ($fichajes as $value)
                        <tr>
                            <td class="centreado">{{ $cont++ }}</td>
                            <td>{{ $value->jugador->full_name }}</td>
                            <td>{{ $value->nro_polera }}</td>
                            <td>{{ $value->fecha_fichaje_t }}</td>
                            <td>{{ $value->contrato_hasta_t }}</td>
